...
#define OPERATOR_COPY_HELPER(op) inline constexpr FlagsTemplate& operator op(const FlagsTemplate& other) noexcept {_bitset op other._bitset; return *this;} inline constexpr FlagsTemplate& operator op(const Bitset& bitset) noexcept {_bitset op bitset; return *this;} inline FlagsTemplate& operator op(const Bits& bit) noexcept {_bitset op bit; return *this;}
#define OPERATOR_CONST_HELPER(op) inline constexpr FlagsTemplate operator op(const FlagsTemplate& other) const noexcept {return _bitset op other._bitset;} inline constexpr FlagsTemplate operator op(const Bitset& bitset) const noexcept {return _bitset op bitset;} inline FlagsTemplate operator op(const Bits& bit) const noexcept {return _bitset op bit;}
