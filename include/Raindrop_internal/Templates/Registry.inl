#ifndef __RAINDROP_INTERNAL_TEMPLATES_REGISTRY_INL__
#define __RAINDROP_INTERNAL_TEMPLATES_REGISTRY_INL__
...
namespace Raindrop::Internal::Templates{
	...
	Registry<T, Handle_t>::Registry(){
		reserve(DEFAULT_SIZE);
	}

	template<typename T, typename Handle_t>
	inline Registry<T, Handle_t>::~Registry(){
		_handles.clear();
		_freeHandles.clear();
	}

	template<typename T, typename Handle_t>
	void Registry<T, Handle_t>::reserve(const std::size_t& n){
	...
	template<typename T, typename Handle_t>
	Handle_t Registry<T, Handle_t>::insert(const std::shared_ptr<T>& instance){
	...
	template<typename T, typename Handle_t>
	void Registry<T, Handle_t>::remove(const Handle& Handle){
	...
	template<typename T, typename Handle_t>
	bool Registry<T, Handle_t>::exists(const Handle& Handle) const noexcept{
	...
	template<typename T, typename Handle_t>
	std::shared_ptr<T> Registry<T, Handle_t>::get(const Handle& Handle) const{
		// query the handle, this also checks that the Handle is in range
		auto& handle = _handles.at(Handle);
	...
	template<typename T, typename Handle_t>
	inline std::size_t Registry<T, Handle_t>::getSize() const noexcept{
		return _handles.size() - _freeHandles.size();
	}

	template<typename T, typename Handle_t>
	inline std::size_t Registry<T, Handle_t>::getCapacity() const noexcept{
		return _handles.size();
	}

	template<typename T, typename Handle_t>
	inline bool Registry<T, Handle_t>::isEmpty() const noexcept{
		return _handles.empty();
	}

	template<typename T, typename Handle_t>
	inline void Registry<T, Handle_t>::collectGarbage() const noexcept{
