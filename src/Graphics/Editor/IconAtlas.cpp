...
namespace Raindrop::Graphics::Editor{
	...
	Icon& IconAtlas::create(const std::string& name, const std::filesystem::path& atlas, const glm::u32vec2& uv1, const glm::u32vec2& uv2){
		CLOG(INFO, "Engine.Graphics.Editor.IconAtlas") << "Registering new icon : " << name << " from " << atlas << " atlas";
		...
		icon->_uv1 = ImVec2(static_cast<float>(uv1.x) / static_cast<float>(texture.width()), static_cast<float>(uv1.y) / static_cast<float>(texture.height()));
		icon->_uv2 = ImVec2(static_cast<float>(uv2.x) / static_cast<float>(texture.width()), static_cast<float>(uv2.y) / static_cast<float>(texture.height()));
