#include <Raindrop/Graphics/BaseFramebuffer/Framebuffer.hpp>
...
namespace Raindrop::Graphics::BaseFramebuffer{
	...
	Framebuffer::Framebuffer(::Raindrop::Graphics::Context& renderer, Maths::uvec2 size) : 
		...
		for (std::size_t i=0; i<4; i++){
			clearColor.color.float32[i] = 0.3f;
		}
	...
	void Framebuffer::swapchainRender(VkCommandBuffer commandBuffer){
	...
	void Framebuffer::beginRenderPass(VkCommandBuffer commandBuffer){
		
		{
			...
			barriers[0].image = _depthAttachment->image;
			...
			barriers[1].image = _colorAttachment->image;
		...
		info.renderArea.extent.width = _size.x;
		info.renderArea.extent.height = _size.y;
	...
	VkRenderPass Framebuffer::renderPass() const{
	...
	void Framebuffer::resize(const Maths::uvec2& size){
		...
		_context->set.update(_colorAttachment->imageView);
	}
