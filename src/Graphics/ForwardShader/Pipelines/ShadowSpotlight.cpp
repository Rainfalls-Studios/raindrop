...
namespace Raindrop::Graphics::ForwardShader::Pipelines{
	ShadowSpotlight::ShadowSpotlight(GraphicsContext& context, VkDescriptorSetLayout descriptorLayout) : _context{context}{
	...
	void ShadowSpotlight::createPipeline(VkDescriptorSetLayout descriptorLayout){
		Builders::GraphicsPipelineBuilder builder;
		builder.addShader(std::static_pointer_cast<Shader>(_context.context.assetManager.loadOrGet("C:/Users/aalee/Documents/raindrop/tests/resources/shaders/forwardShading/spotlight_shadow.glsl.frag.spv").lock()));
		...
		builder.setName("forward shading spotlight with shadow");
		...
		pushConstant.size = sizeof(PushConstants::ShadowSpotlight);
	...
	GraphicsPipeline& ShadowSpotlight::pipeline(){
		return *_pipeline;
	}
}