#include <Raindrop_internal/Graphics/Core/Instance.hpp>
...
namespace Raindrop::Internal::Graphics::Core{
	Instance::Instance(Context& context) : _context{context}{
		...
		_context.getWindow().createSurface();
	}

	Instance::~Instance(){
		_context.getWindow().destroySurface();

		if (_instance){
			vkDestroyInstance(_instance, _context.getAllocationCallbacks());
	...
	void Instance::build(){
		...
		Exceptions::checkVkCreation<VkInstance>(
			vkCreateInstance(&info, _context.getAllocationCallbacks(), &_instance),
	...
	void Instance::requireGraphicsExtensions(){
		auto extensions = _context.getWindow().vulkanExtensions();
